...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        if (raw.date === getToday()) setSt(s => ({ ...s, ...raw, hydrated: true }));
        else setSt(s => ({ ...s, streak: raw.water >= GOALS.water && raw.meals >= GOALS.meals && raw.meds?.every(m => m.taken) ? (raw.streak || 0) + 1 : 0, hydrated: true }));
      } else setSt(s => ({ ...s, hydrated: true }));
    } catch { setSt(s => ({ ...s, hydrated: true })); }
  }, []);

  useEffect(() => { st.hydrated && localStorage.setItem(KEY, JSON.stringify({ ...st, date: getToday() })); }, [st]);
